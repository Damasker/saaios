...
static int touch_get_input_params(void)
{
	...
	pr_info("SAaiOS_TOUCH_DBG: 0x25 GET_TOUCH_REPORT_CONFIG start mode=0x%02x app_status=0x%04x\n",
				...
				&tcm_hcd->config.data_length, NULL, 0);
	{
		...
		hex[0] = 0;
		if (retval >= 0 && tcm_hcd->config.buf) {
			for (i = 0; i < tcm_hcd->config.data_length && i < 8; i++)
				n += scnprintf(hex + n, sizeof(hex) - n, "%s%02x",
					i ? " " : "", tcm_hcd->config.buf[i]);
		}
		pr_info("SAaiOS_TOUCH_DBG: 0x25 GET_TOUCH_REPORT_CONFIG retval=%d timeout=%d len=%u first=%s mode=0x%02x\n",
			retval, retval == -ETIME ? 1 : 0,
			retval >= 0 ? tcm_hcd->config.data_length : 0,
			hex[0] ? hex : "-", tcm_hcd->id_info.mode);
	}
	...
	/* since76: do not auto-send 0x05/0x23/0x26. Menu / sysfs saaios_touch. */
	return 0;
}
...
static int touch_set_input_dev(void)
{
	...
	if (retval < 0) {
		pr_info("SAaiOS_TOUCH_DBG: input/sec_touchscreen register failed retval=%d\n",
			retval);
	...
	pr_info("SAaiOS_TOUCH_DBG: input/sec_touchscreen registered name=%s\n",
		TOUCH_INPUT_NAME);
...
static int touch_set_report_config(void)
{
	...
	pr_info("SAaiOS_TOUCH_DBG: 0x26 SET_TOUCH_REPORT_CONFIG start len=%u mode=0x%02x\n",
		length, tcm_hcd->id_info.mode);
	retval = tcm_hcd->write_message(tcm_hcd, CMD_SET_TOUCH_REPORT_CONFIG,
				...
				NULL, 0);
	pr_info("SAaiOS_TOUCH_DBG: 0x26 SET_TOUCH_REPORT_CONFIG retval=%d timeout=%d mode=0x%02x\n",
...
static int touch_check_input_params(void)
{
	...
	if (touch_hcd->max_x == 0 && touch_hcd->max_y == 0) {
		pr_info("SAaiOS_TOUCH_DBG: max_x=0 max_y=0, using A12s fallback 720x1640 (still register input; 0x25 not claimed OK)\n");
		touch_hcd->max_x = 720;
		touch_hcd->max_y = 1640;
	}
...
static int touch_set_input_reporting(void)
{
	...
	if (IS_NOT_FW_MODE(tcm_hcd->id_info.mode) || tcm_hcd->app_status != APP_STATUS_OK) {
		pr_info("SAaiOS_TOUCH_DBG: 0x05 skip reason=not_fw_or_app mode=0x%02x app_status=0x%04x len=0 (need IS_FW_MODE 0x01|0x02 APP_STATUS_OK)\n",
			tcm_hcd->id_info.mode, tcm_hcd->app_status);
		pr_info("SAaiOS_TOUCH_DBG: skip 0x25/0x05 reporting setup mode=0x%02x app_status=0x%04x (need APP_STATUS_OK)\n",
			tcm_hcd->id_info.mode, tcm_hcd->app_status);
		input_dbg(true, tcm_hcd->pdev->dev.parent,
	...
	if (retval < 0) {
		...
		pr_info("SAaiOS_TOUCH_DBG: 0x25 failed retval=%d, still register input (do not pretend 0x25 succeeded)\n",
			retval);
		if (touch_hcd->max_x == 0 && touch_hcd->max_y == 0) {
			pr_info("SAaiOS_TOUCH_DBG: max_x=0 max_y=0, using A12s fallback 720x1640\n");
			touch_hcd->max_x = 720;
			touch_hcd->max_y = 1640;
		}
		if (touch_hcd->input_dev == NULL) {
			int reg = touch_set_input_dev();
			if (reg < 0)
				pr_info("SAaiOS_TOUCH_DBG: fallback input_register failed retval=%d\n",
					reg);
		}
		goto exit;
	}
...
int touch_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	pr_info("SAaiOS_TOUCH_DBG: touch_init done retval=0 init_touch_ok=%d input_dev=%d sec_touchscreen=%d\n",
		touch_hcd->init_touch_ok ? 1 : 0,
		touch_hcd->input_dev ? 1 : 0,
		touch_hcd->input_dev ? 1 : 0);
...
int touch_init_ok(void)
{
	return (touch_hcd && touch_hcd->init_touch_ok) ? 1 : 0;
}
...
/* Register sec_touchscreen without GET_TOUCH_REPORT_CONFIG 0x25 (no SPI).
 * Dims from 0x20 app_info, else A12s 720x1640. Do not set init_touch_ok.
 */
int touch_register_fallback_input(struct syna_tcm_hcd *tcm_hcd)
{
	int retval;
	unsigned int max_x;
	unsigned int max_y;
	...
	if (touch_hcd->input_dev) {
		pr_info("SAaiOS_TOUCH_DBG: fallback input already registered (no SPI)\n");
		return 0;
	}

	max_x = le2_to_uint(tcm_hcd->app_info.max_x);
	max_y = le2_to_uint(tcm_hcd->app_info.max_y);
	max_objects = le2_to_uint(tcm_hcd->app_info.max_objects);
	if (max_x == 0 || max_y == 0) {
		max_x = 720;
		max_y = 1640;
		pr_info("SAaiOS_TOUCH_DBG: max_x=0 max_y=0, using A12s fallback 720x1640 (no 0x25)\n");
	}
	if (max_objects == 0)
		max_objects = MAX_FINGER_NUM;

	touch_hcd->max_x = max_x;
	touch_hcd->max_y = max_y;
	touch_hcd->max_objects = max_objects;
	touch_hcd->init_touch_ok = false;

	retval = touch_set_input_dev();
	pr_info("SAaiOS_TOUCH_DBG: fallback input (no SPI) retval=%d input_dev=%d max_x=%u max_y=%u objects=%u (0x25 skipped)\n",
		retval, touch_hcd->input_dev ? 1 : 0, max_x, max_y, max_objects);
	if (retval < 0)
		return retval;
