...
/* Normal TCM writes use MIN(max_write, WR_CHUNK_SIZE).
 * CMD_ROMBOOT_DOWNLOAD (0x45) and CMD_DOWNLOAD_CONFIG (0x30): force
 * wr_chunk_size = HDL_WR_CHUNK_SIZE around write_message (0 = one-shot
 * continuous transfer). Do not apply max_write=1024 globally for HDL —
 * CONTINUE_WRITE breaks extended 0x45 (payload=0x17c0e, reserved[0]=0x01;
 * TCM u16 header is 0x7c0e) and LIVE chunked 0x30 (512 then -ETIME).
 */
...
//	bool in_suspend;
	bool irq_enabled;
	bool in_hdl_mode;
	bool romboot_download_deferred;
	bool is_detected;
	bool boot_resume;
