...
# Cross-compile S12 Change 5's boot-attempt/health-check tool for the
# persistent SaaiOS system directory on /data, same as saai-taskd/
# saai-ota-stage/saai-ota-write -- run manually until physically proven
# (ADR-030's "prove it before wiring boot-critical init").
...
binary="$repo_root/target/aarch64-unknown-linux-musl/pixel7/saai-ota-health"
printf '%s\n' "$binary"
sha256sum "$binary"
