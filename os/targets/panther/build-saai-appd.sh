...
# Cross-compile the application lifecycle service for the persistent SaaiOS
# system directory on /data. Unlike displayd and shell, appd is not stored in
# the fixed-size init_boot ramdisk and therefore does not need build-std.
...
binary="$repo_root/target/aarch64-unknown-linux-musl/pixel7/saai-appd"
printf '%s\n' "$binary"
sha256sum "$binary"
