...
/// ADR-402: packed GTK 4.14 Entry inside GtkPopover. Popup then
/// grab_focus. Keyboard-less seat. OSK types `hi!`. Not a Y sweep.
#[test]
fn packed_gtk414_popover_entry_osk_types_hi_bang() {
    let probe = gtk4_alpine_probe();
    let bin = packed_gtk414_popover();
    let loader = probe.join("lib/ld-musl-aarch64.so.1");
    let xkb = probe.join("share/X11/xkb");
    ...
    let mut gtk = Command::new("qemu-aarch64-static")
        ...
        .expect("qemu-aarch64-static failed to spawn gtk414-popover");
    let gtk_out = {
        let stdout = gtk.stdout.take().expect("gtk414-popover stdout");
        let (tx, rx) = mpsc::channel();
        std::thread::spawn(move || {
            let reader = BufReader::new(stdout);
            for line in reader.lines().map_while(Result::ok) {
        ...
        && !saw_kbd_focus)
    {
        ...
        let _ = displayd.wait();
        panic!(
            "popover Entry never enabled v3; popup={saw_popup} failed={popup_failed} enable={saw_enable} ime={:?} kbd={saw_kbd_focus}; displayd={:?}; gtk={stderr}",
            ime_state.activate,
            lines.iter().filter(|l| interesting(l)).collect::<Vec<_>>()
        );
    }
    ...
    assert_eq!(
        entry, "hi!",
        "OSK IME did not type hi! into popover Entry; popup={saw_popup} enable={saw_enable}; displayd={:?}; gtk={stderr}",
        lines.iter().filter(|l| interesting(l)).collect::<Vec<_>>()
    );
}

fn packed_gtk414_combobox() -> PathBuf {
    if let Ok(p) = std::env::var("PACKED_GTK414_COMBOBOX") {
        return PathBuf::from(p);
    }
    gtk4_alpine_probe().join("bin/gtk414-combobox")
}

/// ADR-404: packed GTK 4.14 GtkComboBoxText popup without a tap.
/// gtk_combo_box_popup after map. gtk4-demo combobox class.
/// Not a Y sweep. Not `--run=combobox`.
#[test]
fn packed_gtk414_combobox_popup_maps_xdg_popup_without_click() {
    let probe = gtk4_alpine_probe();
    ...
    assert!(
        saw_toplevel && saw_frame && saw_activated && saw_focus && !saw_kbd_focus,
        "combobox probe did not map/activate; kbd={saw_kbd_focus} focus={saw_focus} xdg={saw_activated} frame={saw_frame} toplevels={n_toplevels}; displayd={:?}; gtk={stderr}",
        lines.iter().filter(|l| interesting(l)).collect::<Vec<_>>()
    );
    assert!(
        saw_popup && !popup_failed,
