...
Host: `cargo test -p saai-taskd` 79 passed, including
`next_admission_is_none_while_mutating_runs` and
`next_admission_unblocks_child_after_parent_done`. Panther: binary
`e0365939…` pid 852. Do not type an Intent; do not tap Разрешить.
Marker on.
