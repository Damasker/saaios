...
A second, longer effort is now in scope: a native OS that can boot on a phone
(primary target: Google Pixel 7 `panther`), later on Pi 5 and x86. Mixing that
into Platform crates would stall both tracks.
...
- Google Pixel 7 (`panther`) is the primary hardware target, not the OS.
  Future backends remain `raspberry-pi` and `x86`.
