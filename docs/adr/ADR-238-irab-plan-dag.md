...
Host: `cargo test -p intent-resolution -p saai-taskd` including
`structured_plan_is_not_a_diagnose_fallback`,
`bind_plan_remaps_proposal_ids_in_topo_order`,
`plan_does_not_admit_sibling_while_confirming`. Panther: flash
`saai-taskd`; do not type an Intent; do not tap Разрешить. Marker on.
