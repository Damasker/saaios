...
This is a **host USB stick** driver. SM-A127F onboard radio is Samsung/Exynos SCSC Maxwell `wlan0` (`119c0000.scsc_wifibt`, `mx140.bin` in `/vendor/etc/wifi`). Phase E3 is that path — **v029 LIVE** (wifi-up, scan, Wallbox join at `192.168.168.8`); **v031 packed** (`/sbin/wpa_supplicant` + `/sbin/wifi-join` by args, not flashed). No PSK in the image. See [hardware-control-plan.md](hardware-control-plan.md). Not this chip. Next plan item is E4.
...
| **Phone (Phase E radios)** | Onboard = Maxwell / `wifi-up` / `wifi-join` (E3, v029 LIVE join; v031 packed). An 8188 stick needs USB **host**/OTG and drops RNDIS. Stock DXJ2 has `CONFIG_USB_DWC3_DUAL_ROLE`. v028 `/sbin/usb-host` is explicit; boot stays gadget. See [hardware-control-plan.md](hardware-control-plan.md) E2 then E3. Next is E4. |
