...
since77 is the current known-good baseline (read-floor fix engaged, REINIT's `0x20` succeeds). since78 confirmed the post-REINIT wall is opcode-independent (`0x05` dies identically to `0x20`). since79's `identify(true)` experiment is **falsified and must not be repeated as-is** — the driver's `identify()` call at the REINIT site should read `identify(tcm_hcd, false)` again (since77/78's version) before any further work builds on top of it. No dmesg capture possible on this unit without root (`ro.debuggable=0`); Magisk root attempted and abandoned (DEFEX suspected, see above); the touch-lab kernel's `KernelSU-Next` in `LOCALVERSION` is a dead string, not real (checked, no source in tree).

1. **Revert the identify(true) change first.** Any new sinceN branches from since77/78's `identify(false)`, not since79.
2. **Reference driver diffing is now a real, source-backed lead — keep using it, but verify claims LIVE before trusting them.** [ASKSAP/android_kernel_samsung_a12](https://github.com/ASKSAP/android_kernel_samsung_a12) is a genuine independent Synaptics TCM driver tree (MediaTek A125F, not this Exynos A127F — board/platform glue will differ, but the core TCM/HDL/zeroflash protocol logic is the same driver family and has already correctly predicted two things this file got right from OSS-only analysis: 0x25/0x26 usage and 0x05 testing-only). since79 shows it can also predict something that turns out **wrong** for this unit/firmware — verify every reference-driven hypothesis LIVE, same as any other. Unexplored in that tree: `zeroflash_syncbox()`, `synaptics_tcm_recovery.c`, `synaptics_tcm_diagnostics.c`, resume/`AFTER_RESUME` flow, and whether there is a settle delay between the leftover `0x1b` IDENTIFY and REINIT that this unit's firmware needs before it will accept a *second* kind of command.
3. **Do not repeat blind opcode/timing experiments with no reference or reasoning** — `0x05`/`0x24`/`0x30`/`0x26`/`0x02` immediately after REINIT are all now proven to jam or get zero response, regardless of delay (10 ms is already fatal). Any new hypothesis needs a specific reason (a reference-driver call, a state/timing detail not yet tested) beyond "try a different opcode."
4. Do **not** flash since66, since73, since75, since76, since77, or since78. Do **not** rewind 0x45 oneshot / skip 0x1f / leftover REINIT 0x20 / the since77 read-floor. Do **not** retry 0x26 / auto 0x05/0x25/0x30 / `identify(true)` at the REINIT site.
