...
(ADR-029 + ADR-030). Дальше -- Change 2: минимальный вертикальный срез
(одна Intent-строка -> один Task с одним неопасным Action -> Result,
видимый в `Сейчас`); состояния и идемпотентность на реальных записях;
подтверждение опасного Action, переживающее cold-reboot.
