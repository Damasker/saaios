...
staged-загрузка не доходит до записи; отказ на любом шаге не портит
активный слот (backup/restore drill); намеренно нездоровая тестовая
сборка запускает boot attempt limit и автоматический откат;
обновлённый образ переживает настоящий cold reboot. Честно
зафиксировано: переход слота B и настоящий cross-slot A/B hardware
failover не проверялись и не входят в этот спринт.
