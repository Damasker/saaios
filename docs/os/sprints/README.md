...
surface, Object View, apps grid, Inbox header, Spaces header, Система header, consent header, PIN setup header, remote pairing header, Bluetooth header, Wi-Fi header, trusted-clients header, lock attention и lock/PIN keyboard на panther). VUI-02 остаётся почти закрытым (шрифты в boot-image —
