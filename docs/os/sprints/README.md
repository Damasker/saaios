...
[Visual System delivery roadmap](VISUAL-ROADMAP.md): VUI-00, VUI-01 и
VUI-03 закрыты; VUI-02 принят кроме одного пункта, заблокированного
окружением (шрифты в boot-image без stock firmware);
**VUI-04 выполняется**. Этот трек не переоткрывает выполненные
