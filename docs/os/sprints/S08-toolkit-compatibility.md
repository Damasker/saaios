...
- Архитектурные решения: ADR-021 (источник пакетов -- Alpine musl, не
  ...
  wl_shm-буфера -- дефект апстрима, локализован, не устранён; GTK4
  заблокирован для физической приёмки Change 7 до отдельного решения
  (gdb-отладка на устройстве, другая сборка GTK4, либо Qt/Kirigami как
  основной toolkit). ADR-026: тем же методом физически подтверждено,
