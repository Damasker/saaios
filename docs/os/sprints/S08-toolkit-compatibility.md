...
- Состояние: `Done (переоцененный объём, 2026-09-11)` -- GTK4 снят из
  критического пути отдельным решением (ADR-025), Qt/Kirigami принят
  как достаточное доказательство Goal вместо "обоих toolkit'ов
  одновременно". Полное обоснование -- Acceptance criteria ниже.
- Зависит от: S07 (`Done`).
- Архитектурные решения: ADR-021 (источник пакетов -- Alpine musl, не
  ...
  ADR-025: тот же software-путь у GTK4 (Alpine's `gtk4.0` 4.14.4)
  детерминированно падает на реальном железе (не в qemu) при создании
  wl_shm-буфера. Точная причина найдена через `gdb` на устройстве
  (2026-09-11, ADR-025 обновлён): `gdk_wayland_display_create_shm_
  surface`, повреждённое значение `height`, `scale` передан указателем
  -- вероятно неинициализированная переменная масштаба из-за
  отсутствующего `wp-fractional-scale-v1`. Исправление (патч GTK4 или
  реализация протокола) не сделано, GTK4 остаётся заблокирован для
  физической приёмки Change 7. ADR-026: тем же методом физически подтверждено,
  что ни Qt5/QtQuick, ни Kirigami2 не унаследуют этот дефект -- оба
  рендерят и коммитят реальный кадр на том же железе, включая против
  боевого `saai-displayd` -- Change 7 переключается на Qt/Kirigami как основной
  toolkit. ADR-027: `org.saaios.demo.kirigami` -- первое реальное,
  воспроизводимо собираемое (`build-kirigami-demo-package.sh`)
  приложение стороннего toolkit'а прошло install→launch→stop→remove
  через настоящий `saai-appd`'s IPC на боевом устройстве. ADR-028: touch
  физически подтверждён отдельно -- синтетическая evdev-инъекция через
  `uinput` (безопасно, в изолированном `unshare -m`-namespace, без
  риска для боевого стека) доставила тап именно на Kirigami-приложения
  surface ID, включая корректную маршрутизацию через lock-screen
  (ADR-015). Негативный sandbox-тест на clipboard для этого конкретного
  приложения остаётся непроверенным (общий пробел уже задокументирован
  отдельно, ADR-023).
- Рабочий fallback: `saai-shell` и `saai-demo-surface` продолжают быть
  единственными постоянно установленными Wayland-клиентами;
  `org.saaios.demo.kirigami` физически проверен, но удалён после
  проверки (ADR-027), не оставлен установленным на устройстве -- пока не
...
7. Один GTK-демо и один Qt/Kirigami-демо приложение, физическая приёмка на
   ...
   **Расследование телефонного крэша завершено (2026-09-10, ADR-025,
   без кодовых изменений в проекте -- дефект локализован в апстриме).**
   Последовательно исключены все причины на стороне проекта: не пробел
   в упаковке (тот же минимальный `bin/`+`lib/`+xkb-пакет рендерит и
   коммитит кадр под `qemu-aarch64-static`); не `saai-displayd`'s
   hardware-backend или keyboard-код (throwaway headless-без-клавиатуры
   `saai-displayd`, кросс-компилированный и физически запущенный на
   реальном телефоне, крашит `gtk4-demo` идентично -- то есть падает
   ЛЮБОЙ compositor на этом железе, не только `panther-hardware`); не
   зависит от сложности окна (`gtk4-demo --run=dialog`, минимальный
   демо, падает так же); не про pixman's NEON-диспетчер
   (`PIXMAN_DISABLE=arm-simd,arm-neon,arm-iwmmxt` не помогает); не общая
   проблема memfd/shm на ядре (throwaway `memfd-probe.c` через `zig cc`
   -- `memfd_create`+`ftruncate`+`mmap` успешны вплоть до реального
   размера экрана, 10 368 000 байт). Дефект детерминированно
   воспроизводится на РЕАЛЬНОМ aarch64 (Tensor G2) во всех
   протестированных конфигурациях compositor'а и НИКОГДА -- под
   `qemu-aarch64-static` с идентичными бинарниками -- указывает на
   архитектурно-специфичный баг в самом GTK4/GDK's Wayland-backend
   (Alpine's `gtk4.0` 4.14.4 aarch64-сборка), не в коде проекта.
   Дальнейшая локализация требует `gdb`/`rr` на устройстве или
   бисекции версий GTK4/Cairo -- вне объёма этого расследования, вне
   доступного инструментария (serial-console/qemu). Полный ход и все
   исключённые гипотезы -- ADR-025.

   **Точная причина найдена (2026-09-11, ADR-025 обновлён).** Alpine's
   `gdb`+`gtk4.0-dbg` пакеты установлены и физически развёрнуты тем же
   безопасным `unshare -m`-паттерном -- `gdb -batch -ex run -ex bt`
   против `gtk4-demo --run=dialog` дал точный, символизированный
   backtrace: `gdk_wayland_display_create_shm_surface(width=508,
   height=1776831, scale=<pointer>)` at `gdkdisplay-wayland.c:1472`.
   `height` явно повреждён; `scale` передаётся указателем -- сильный
   структурный намёк на неинициализированную переменную масштаба,
   разыменованную где-то в цепочке вызовов. Правдоподобно увязывается с
   уже известным фактом: `saai-displayd` не реализует
   `wp-fractional-scale-v1` -- GDK, вероятно, инициализирует scale
   только по событию этого протокола, и без него переменная остаётся
   мусорной на стеке, что естественно объясняет и находку "никогда не
   воспроизводится под qemu" (разный мусор на стеке в разных средах
   исполнения). Исправление (патч/пересборка GTK4, либо реализация
   `wp-fractional-scale-v1` в `saai-displayd` с последующей физической
   проверкой) не сделано -- вне критического пути Change 7, который уже
   закрыт через Qt/Kirigami.

   **Qt5/QtQuick проверен тем же методом (2026-09-10, ADR-026) --
   рендерит корректно там, где GTK4 падает.** Тот же throwaway
   headless-без-клавиатуры `saai-displayd` + `unshare -m`/`mount
   --bind`-воспроизведение, минимальный QML-файл (один `Rectangle`, без
   Kirigami). После добавления недостающих `usr/lib/qt5/plugins/`
   (platform-плагин, `xdg-shell`-интеграция -- оба пробела чисто
   упаковочные, аналог `usr/share/X11/xkb`) -- EGL закономерно
   недоступен (ADR-024), но Qt тихо откатывается на СВОЙ собственный
   software/shm rendering-путь (не через Cairo -- отдельная от GTK
   реализация) и коммитит реальный кадр
   (`frame sha256=761a9e67...`), без единого крэша. Воспроизведено
   дважды подряд (идентичный хэш) и повторно против РЕАЛЬНОГО боевого
   `saai-displayd` (`panther-hardware`, тот самый процесс, против
   которого падал GTK4 в Change 6) -- тот же чистый результат,
   compositor пережил тест без перезапуска. Change 7 переключает
   основной toolkit-таргет на Qt/Kirigami; GTK4 остаётся blocked
   (ADR-025), не отменяет задачу, но не в критическом пути приёмки
   этого Change.

   **Kirigami2 конкретно тоже подтверждён тем же методом (2026-09-10,
   там же ADR-026, обновлён).** Переиспользован `kirigami-spike.qml` из
   ADR-021's исходного спайка. Сверх Qt5's `plugins/` потребовался весь
   `usr/lib/qt5/qml/` (13 MiB, не только `QtQuick` -- Kirigami тянет
   `QtQuick.Controls`/`Layouts` транзитивно). `kf.kirigami: Failed to
   find a Kirigami platform plugin` -- не найден в Alpine-закрытии
   зависимостей вовсе, но не фатально, Kirigami продолжает с дефолтным
   поведением. Реальный `xdg_toplevel` создан, реальный кадр
   закоммичен (`frame sha256=b90311b0...`), воспроизведено дважды
   (идентичный хэш) и повторно против боевого `saai-displayd` -- без
   единого крэша, compositor пережил тест без перезапуска. Change 7's
   основное демо теперь нацелено конкретно на Kirigami2, не
   гипотетически.
...
Спринт закрыт в переоцененном объёме (по аналогии с Change 4's
собственным прецедентом внутри этого же спринта) -- ниже указано, что
физически подтверждено, и что явно снято/заменено отдельным ADR, а не
молчаливо недоделано:

- ~~GTK-приложение~~ **и** Qt/Kirigami-приложение запускаются,
  масштабируются, получают touch, переживают switch и закрываются --
  **GTK-часть снята**: детерминированный, воспроизводимый крэш апстрима
  на реальном железе, локализован до точной строки через `gdb`
  (ADR-025), не устраним в рамках проекта. **Qt/Kirigami-часть
  подтверждена**: `org.saaios.demo.kirigami` устанавливается, запускается,
  сосуществует с `org.saaios.demo-surface` (switch на уровне протокола),
  корректно останавливается/удаляется (ADR-027); touch физически
  подтверждён синтетической evdev-инъекцией, включая доставку именно на
  Kirigami-surface (ADR-028). "Масштабируются" в буквальном смысле
  (resize/другой scale factor) не тестировалось отдельно -- вне
  проверенного объёма.
- системные разрешения нельзя обойти через toolkit-специфичный API --
  **снято для этого спринта**: сама возможность гейтить нативный
  Wayland-clipboard через capability-модель architecturally недостижима
  в текущей версии smithay (ADR-023, нет точки перехвата ни для записи,
  ни для чтения) -- не проверено для `org.saaios.demo.kirigami`
  специально, т.к. `app.qml` не реализует clipboard вовсе, и общий
  пробел уже не специфичен для одного приложения.
- размер и память каждого демо-приложения измерены -- **подтверждено**
  для Kirigami: 249 MiB на диске (89 MiB сжатый для переноса),
  `VmRSS` реального запущенного процесса -- 61 028 KB (~60 MiB).
- намеренно повреждённый Wayland-клиент этих toolkit'ов отключается без
  падения `saai-displayd` -- **подтверждено** дважды независимо: (1)
  GTK4's `gtk4-demo` крашился (`SIGSEGV`) десятки раз за время
  расследования ADR-025 против нескольких инстансов `saai-displayd`
  (headless и `panther-hardware`) -- ни один не упал; (2) целевой
  `kill -9` работающего `org.saaios.demo.kirigami` (pid 1726) --
  `saai-displayd` (pid 612, тот же хэш `541a5eeb...`) не затронут,
  `saai-appd` корректно перезапустил приложение (новый pid 1735, тот же
  `single_instance`-механизм, что и остальные приложения S05).
...
Change 7: GPU-driver spike (ADR-024), крэш-расследование GTK4 (ADR-025)
и физическое подтверждение Qt5/Kirigami2 (ADR-026) -- см. записи выше в
этом же Change. Основная работа -- полный ход и обоснование ADR-027.
Кратко: собран `org.saaios.demo.kirigami` -- первое приложение в
проекте с рантаймом не из собственных исходников репозитория (в отличие
от `demo-surface`). Новые закоммиченные артефакты:
`apps/kirigami-demo/{manifest.toml,app.qml,README.md,launch.c}` (`launch.c`
-- статический aarch64-musl враппер, `zig cc`, нужен т.к. `AppManifest`'а
`exec`-поле не поддерживает argv/env) и
`os/targets/panther/build-kirigami-demo-package.sh` (тот же паттерн, что
`build-demo-package.sh`, но тянет рантайм через `apk-tools-static`/
`apk add kirigami2` вместо `cargo build`, explicit trust-модель в
комментарии скрипта). Скрипт физически прогнан на R620 дважды подряд
(идемпотентен) -- готовый пакет 249 MiB/89 MiB сжатый (заметно меньше
507 MiB ручных throwaway-сборок этого дня -- те тащили Qt6/GTK4/
mesa-dri-gallium/LLVM транзитивно из общего sysroot'а, этот пакует ровно
`kirigami2`'s собственное закрытие).

Физически на устройстве, через настоящий `saai-appd`'s IPC (не
`unshare -m`-обход, как в ADR-026): `install` -- успешно; `launch` --
реальный `qmlscene-qt5 .../app.qml` процесс (`execv()` внутри `launch`
корректно заменил образ), состояние `running`, не падает, не
`crash_limited`; параллельно запущен `org.saaios.demo-surface` -- оба
сосуществуют (`list` показывает оба `running`); `stop` -- чисто
останавливает Kirigami-приложение, `demo-surface` не затронут; `remove`
-- пакет удалён, `list` подтверждает возврат к исходному состоянию.
Устройство и R620 очищены от scratch-файлов.

Touch физически подтверждён отдельно (2026-09-10, ADR-028) --
`panther-hardware`-сборка `saai-displayd` не имеет синтетического
touch-инжектора и serial-консоль не может коснуться экрана, но
`touch.rs` читает `/dev/input/touchscreen` как сырой evdev, что открыло
путь к инъекции через Linux's `uinput`: виртуальное touch-устройство
(созданное вручную -- `/dev/uinput` и результирующий `/dev/input/eventN`
не появляются автоматически, нет hotplug-демона) подставлено вместо
реального тачскрина только внутри изолированного `unshare -m`
mount namespace throwaway `panther-hardware`-инстанса `saai-displayd`
(его неудачный захват DRM, поскольку реальный compositor уже держит
master, обрабатывается штатно -- не падает, продолжает поднимать touch
и Wayland). Два синтетических тапа физически подтвердили полный путь:
первый ушёл на lock-surface (`locked=true`, ADR-015's инвариант) и
разблокировал реальный `saai-shell`; второй, после разблокировки, ушёл
именно на `wl_surface@15[1], 24` -- тот же ID, что был toplevel'ем
`org.saaios.demo.kirigami`'s процесса чуть раньше в том же логе. Полное
совпадение ID -- прямое доказательство доставки touch конкретно в
Kirigami-приложение. Реальный боевой `saai-displayd`/`saai-appd`
подтверждены неизменными до и после. Негативный sandbox-тест на
clipboard не переприменён к этому приложению -- `app.qml` не реализует
clipboard-операции, и известный пробел (ADR-023) уже архитектурно
задокументирован отдельно от конкретного приложения.

Закрытие спринта (2026-09-11): переустановлен, запущен
`org.saaios.demo.kirigami` через настоящий `saai-appd`'s IPC. `VmRSS`
реального процесса -- `61 028 KB` (`/proc/<pid>/status`). `kill -9`
работающего процесса (pid 1726) -- `saai-displayd` (pid 612, хэш
`541a5eeb...`) не затронут, `saai-appd` корректно перезапустил
приложение (новый pid 1735, состояние `running`) -- тот же
crash-recovery механизм, что и у остальных S05-приложений. Отдельно,
намеренно повреждённый GTK4-клиент (десятки `SIGSEGV` за время
расследования ADR-025, против нескольких инстансов `saai-displayd`)
ни разу не уронил compositor -- та же инвариант, доказанная другим
путём. Приложение остановлено и удалено, устройство подтверждено
возвращённым к исходному состоянию (только `org.saaios.demo-surface`).
