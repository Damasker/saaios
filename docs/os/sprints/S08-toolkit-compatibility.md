...
- Архитектурные решения: ADR-021 (источник пакетов -- Alpine musl, не
  ...
  что Qt5/QtQuick НЕ унаследует этот дефект -- рендерит и коммитит
  реальный кадр на том же железе, включая против боевого
  `saai-displayd` -- Change 7 переключается на Qt/Kirigami как основной
  toolkit.
...
7. Один GTK-демо и один Qt/Kirigami-демо приложение, физическая приёмка на
   ...
   **Qt5/QtQuick проверен тем же методом (2026-09-10, ADR-026) --
   ...
   этого Change. Kirigami2 конкретно (не голый QtQuick) этим ADR не
   проверен -- часть основной работы Change 7.
