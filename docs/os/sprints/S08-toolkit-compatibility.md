...
- Архитектурные решения: ADR-021 (источник пакетов -- Alpine musl, не
  ...
  через настоящий `saai-appd`'s IPC на боевом устройстве; touch
  физическим тапом и негативный sandbox-тест на clipboard для этого
  конкретного приложения остаются непроверенными в рамках сессии (см.
  ADR-027's "Последствия").
...
Touch физически не проверен тапом в рамках этой сессии --
`panther-hardware`-сборка `saai-displayd` не имеет синтетического
touch-инжектора (в отличие от headless-сборки's `inject-key`), а
serial-консоль не может физически коснуться экрана; достоверно только
унаследованное от S02/S03 touch-протокольное покрытие. Негативный
sandbox-тест на clipboard не переприменён к этому приложению -- `app.qml`
не реализует clipboard-операции, и известный пробел (ADR-023) уже
архитектурно задокументирован отдельно от конкретного приложения. Полное
обоснование обоих пунктов -- "Последствия" в ADR-027.
