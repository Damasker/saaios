...
- **Статус**: частично проверено, 2026-09-17, ADR-097. Не Done -- см.
  точный блокер ниже.
...
- **Новый открытый вопрос** (не решается этим пунктом): нужен ли
  SaaiOS собственный минимальный session D-Bus как системный сервис --
  затрагивает `services/saai-appd/src/sandbox.rs` и модель
  capability'ей, вероятно нужен не только для APP-05, но и для APP-06
  и почти любого стороннего Linux-приложения сложнее демо. Требует
  отдельного Definition of Ready прохода, не решён здесь.
...
APP-04  экранная клавиатура поверх APP-03
APP-05  PCManFM-Qt (файловый менеджер) -- ADR-097, частично: рендерит, D-Bus daemon отсутствует
