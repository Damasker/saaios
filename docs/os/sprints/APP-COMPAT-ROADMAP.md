...
APP-04  экранная клавиатура -- Keyboard→IME (ADR-270 host); GTK 4.18 Entry v3 enable (ADR-322) and OSK types hi! (ADR-325); Qt 5.15 v2 enable (ADR-323); IME forwarded, Filter not focused (ADR-324); host Ctrl+I disables v2 (ADR-326); PathEdit click re-enables v2, still no shm paint (ADR-327); host Qt 5.15 QLineEdit types hi! (ADR-328); panther field not typed
