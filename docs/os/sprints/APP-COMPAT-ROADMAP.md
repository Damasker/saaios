...
### APP-00: musl vs glibc -- закрыть вопрос спайком, не предположением
...
- **Текущее состояние**: ADR-021 уже выбрал musl/Alpine для тулкитов;
  ни одна зависимость, требующая glibc, пока не названа.
- **Host-тестируемо**: да -- вопрос решается чтением/сборкой пакетов,
  не требует телефона.
- **Приёмка**: ADR, явно говорящий "musl, если не найдено обратное" ИЛИ
  называющий конкретный blocker.

### APP-01: Qt hello-world под текущим `saai-appd` -- вероятно, уже де-факто закрыт
...
- **Приёмка**: `org.saaios.demo.kirigami` (или эквивалент) снова
  ...
  изменений с S08) -- подтвердить, что регрессий не было.
...
- **Приоритет**: PCManFM-Qt (Qt6, не блокирован ADR-025) впереди
  Nautilus (GTK4, блокирован до APP-02).
- **Зависит от**: APP-00 (musl-путь для Qt6, если ещё не покрыт S08's
  Qt5-прецедентом -- нужно подтвердить, что Qt6 не вносит новых
  проблем).
...
```text
APP-00  musl vs glibc -- ADR
APP-01  Qt hello-world -- переподтвердить существующий ADR-026/027/028
...
APP-04  экранная клавиатура поверх APP-03
APP-05  PCManFM-Qt (файловый менеджер)
