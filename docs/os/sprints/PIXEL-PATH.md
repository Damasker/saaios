...
| UAM | 124 | `saai-authority` + live `decide_named` + AUTH-02/03 host | `TaskConfirm` живой; AUTH-04 host |
...
- APP-COMPAT APP-04 OSK is existing Keyboard via IME `commit_string` (ADR-270; host). Layer size/hit ADR-271 host. No wvkbd. Panther chrome waits.
