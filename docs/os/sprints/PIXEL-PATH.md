...
- APP-COMPAT APP-04 OSK is existing Keyboard via IME `commit_string` (ADR-270; host). Layer size/hit ADR-271, blit dest ADR-272, shell IME layer ADR-273. Panther displayd `02c78f9f…` advertises IME v2 (ADR-309); shell `4dc19018…` no longer logs unavailable. Falkon `file://` `<input>` does not Activate IME (ADR-317). PCManFM-Qt Filter `QLineEdit` is on panther and still does not Activate (ADR-318). Root cause: Qt binds `zwp_text_input_v2`, compositor was v3-only; host now advertises v2 (ADR-319; no displayd flash). Host GTK 4.18 `Entry` sends `zwp_text_input_v3::enable` (ADR-322). No wvkbd.
