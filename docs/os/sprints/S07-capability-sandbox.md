...
8. **Готово (2026-09-10, до `5b99b01`, cold reboot -- отдельно, без
   кодовых изменений).** Физическая приёмка на устройстве: негативные тесты/fault
   ...
   Cold reboot grants -- физически прогнан отдельно (см. Evidence):
   две throwaway-заявки (одна принята с `net.internet`, другая явно
   отклонена с `clipboard.read`) пережили настоящую перезагрузку
   (`reboot -f`, `uptime` подтвердил `up 0 min`) с побайтово идентичными
   файлами решений и без единого нового `decide_consent`.

   Побочная находка при этом прогоне (не про grants, про деплой): у
   `saai-shell` (в отличие от `saai-appd`, который живёт на `/data` --
   настоящем персистентном разделе) hot-swap на `/saaios/saai-shell` не
   переживает реальную перезагрузку -- `/saaios` часть корня, который
   при каждой загрузке пересоздаётся заново из прошитого `init_boot`,
   так что после `reboot -f` там снова оказывается старый образ без
   portal-сокета. Это ожидаемое следствие уже задокументированного в
   `saai-shell`'s doc-comment паттерна ("без пересборки init_boot"), не
   баг -- после реальной перезагрузки `saai-shell` нужно передеплоить
   тем же hot-swap'ом заново перед тем, как sandbox-зависимые вещи
   (portal-сокет) снова заработают; `saai-appd`'s grants и sandbox --
   это не затрагивает никак.
...
Change 8 (cold reboot): без кодовых изменений -- чисто device-level
acceptance прогон. Установлены две throwaway-заявки: `org.saaios.
test.reboot-grant-probe` (`net.internet`, согласие принято) и
`org.saaios.test.reboot-decline-probe` (`clipboard.read`, согласие явно
отклонено). До перезагрузки записан sha256 обоих файлов решений в
`/data/saaios/var/appd/grants/` и полный `list()`. Выполнен `sync`,
затем реальная перезагрузка устройства (`reboot -f`; обычный `reboot`
без `-f` не подействовал, `busybox reboot --help` подтвердил, что
бинарь рабочий -- не расследовалось глубже, раз `-f` сработал);
`uptime` после возврата серийной консоли показал `up 0 min` -- честный
холодный старт, все `saai-*` сервисы подняты заново init'ом с новыми
pid.

После перезагрузки: sha256 обоих файлов решений в `/data/saaios/var/
appd/grants/` побайтово совпал с записанным до перезагрузки -- durable
fsync-персистентность (тот же паттерн, что и в S06) пережила реальный
холодный старт, не только `kill`. `list()` без единого нового
`decide_consent` вернул то же самое: `reboot-grant-probe` --
`consent_needed: false`, `granted_capabilities: ["net.internet"]`;
`reboot-decline-probe` -- `consent_needed: false`, `granted_capabilities:
[]` -- принятое решение не откатилось к запросу заново, отклонённое не
подменилось молчаливым разрешением.

`launch` на `reboot-grant-probe` после передеплоя `saai-shell` (см.
находку про init_boot выше) сразу вернул `launched`, не `consent_
required` -- персистентный grant реально используется при запуске, не
только отражается в `list()`. Собственный отчёт `sandbox-probe` внутри
`reboot-decline-probe` после перезапуска показал чистые `RESULT PASS
failures=0` по всем 18 пунктам, включая `network unavailable without
net.internet` -- приложение без `net.internet` остаётся без сети и
после холодной перезагрузки. Обе throwaway-заявки удалены, устройство
возвращено к исходному состоянию (`list()` показывает только
`org.saaios.demo-surface`).
