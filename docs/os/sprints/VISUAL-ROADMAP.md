...
| VUI-07 | Remaining system surfaces and state patterns | **In progress** (Inbox + Spaces + Wi-Fi + Bluetooth + trusted clients + Wi-Fi password + PIN setup + lock idle + intent input + developer surface + Object View + apps grid + Inbox header + Spaces header + Система header + consent + PIN setup header + remote pairing + Bluetooth header + Wi-Fi header + trusted-clients header on panther) |
...
header (`efbab13a…`), Wi-Fi header (`d562249b…`), and trusted-clients
header (`b73f9433…`) are on panther. Space detail still deferred. MEM-08 stays omitted until a
