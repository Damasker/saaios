...
| VUI-07 | Remaining system surfaces and state patterns | **In progress** (Inbox + Spaces + Wi-Fi + Bluetooth + trusted clients + Wi-Fi password + PIN setup + lock idle + intent input on panther; developer surface host-green) |
...
**Status:** In progress (Inbox `c1547c02…`, Spaces `9bb75db5…`, Wi-Fi `c80bb666…`, Bluetooth `5b37c5bc…`, trusted clients `cd207b18…`, Wi-Fi password `0eeb36d3…`, PIN setup `eb4cb508…`, lock idle `a19327cb…`, intent input `6239bebd…` on panther; developer surface host-green ADR-136)
...
- [x] Migrate the hidden developer surface onto Static `DataRow`.
  ...
  remote pairing / PIN keypad chrome still later. Host-green;
  panther pending.
...
(`6239bebd…`) are on panther. Developer surface is host-green
(ADR-136). Space detail still deferred. MEM-08 stays omitted until
