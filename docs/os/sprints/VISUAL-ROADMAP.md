...
| VUI-09 | `.sui` v2, public library, legacy cleanup, and release gate | In progress (ADR-180–210; `compile()` stays v1; not Visual v1 sign-off) |
...
- [x] Migrate `Bluetooth устройства` to live scan rows. ADR-130:
  `BluetoothRow` from `bt-scan`; paired from `SAVED` names; empty
  only after `DONE`. Flattened to `ActionCardView`. Trusted clients /
  lock still later. Flashed `5b37c5bc…` (X5 BLE + OBDII CLASSIC).
- [x] Migrate `Доверенные клиенты` to live `authorized_keys` rows.
  ADR-131: `TrustedClientRow` from the file; SHA256 prefix not the
  raw key; empty «Нет клиентов». Flattened to `ActionCardView`.
  Wi-Fi password / lock still later. Flashed `cd207b18…`.
- [x] Migrate the Wi-Fi password keyboard onto `Field`. ADR-132:
...
- [x] Migrate the hidden developer surface onto Static `DataRow`.
  ADR-136: live facts; label and value apart; title below the
...
- [x] Migrate Object View onto `ObjectSummary`. ADR-137: identity
  ...
  keypad chrome still later. Flashed `e2081d84…`.
- [x] Migrate the `Приложения` grid off `draw_root`. ADR-138:
  `ContextHeader` section `Приложения`; live `installed_apps` only;
...
- [x] Migrate Пространства off `draw_root`. ADR-140: `ContextHeader`
...
- [x] Migrate Система off `draw_root`. ADR-141: `ContextHeader` section
  ...
  `15fc3487…`.
- [x] Migrate app-consent off the free-floating title. ADR-142:
...
- [x] Migrate remote pairing off the free-floating title. ADR-144:
...
- [x] Migrate Bluetooth list off the Surface header fill. ADR-145:
  ...
  `efbab13a…`.
- [x] Migrate Wi-Fi list off the Surface header fill. ADR-146:
  `ContextHeader` section `Wi-Fi`; live WifiRow cards plus
  refresh/back unchanged. Do not tap a network. Do not type a PSK.
  Leave with Назад. Trusted / DevSurface chrome still later. Flashed
  `d562249b…`.
- [x] Migrate trusted clients off the Surface header fill. ADR-147:
...
- [x] Paint essential attention on the no-PIN lock. ADR-148:
  compact `StatusIndicator` from store-connected + `has_orb_attention`
...
- [x] Paint live device state on the no-PIN lock. ADR-153: Caption
...
(ADR-209); display restart (ADR-210). Release
...
- [x] Prove display restart on panther HEAD (ADR-210; kill unlocked
  ...
  7-tap gallery stay open.
- [ ] Run remaining live matrix cells (cold boot, 7-tap gallery)
  then close the ledger. Daylight booth stays deferred (ADR-193).
...
operator-approved cold boot / 7-tap gallery. Lock cycle is proven
(ADR-209). Display restart is proven (ADR-210). Leftover text sizes
