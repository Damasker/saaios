...
| VUI-07 | Remaining system surfaces and state patterns | **In progress** (Inbox + Spaces + Wi-Fi on panther; Bluetooth list host ADR-130) |
...
**Status:** In progress (Inbox `c1547c02…`, Spaces `9bb75db5…`, Wi-Fi `c80bb666…` on panther; Bluetooth list host ADR-130)
...
list (`9bb75db5…`), and Wi-Fi list (`c80bb666…`) are on panther.
Bluetooth list is host-only until flashed. MEM-08 stays omitted until
